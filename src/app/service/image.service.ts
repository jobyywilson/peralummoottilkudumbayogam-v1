...
public getUserPhotoUrl(userId:string){
        ...
        let defaultPic = DEFAULT_PROFILE_PIC
        let profilePic = defaultPic
        let spousePic =  defaultPic
        if(photoName){
        ...
        else if(userId.endsWith("S") && photoName){
          spousePic = "https://image.peralummoottilkudumbayogam.org/"+photoName;
        }
        else{
          spousePic = DEFAULT_PROFILE_PIC;
        }
...
public getUserPhotos(url: string): any {
  if(!url || url == DEFAULT_PROFILE_PIC){
    url =   USER_IMAGE;
    return url;
  }
