...
const RELATIONSHIP_URL = 'https://hzqpjqvopcevhucgazlh.supabase.co/rest/v1/relationship_undeployed'
...
exports.handler = (event, context, callback) => {
    ...
    const myHeaders = {
      ...
      if(isSpouseNodeId(nodeId)){
        url = `${RELATIONSHIP_URL}?secondary_member_id=eq.${nodeId}&select=spouse:relationship_undeployed_primary_member_id_fkey(nodeId,name)`
      }else{
        url = `${RELATIONSHIP_URL}?primary_member_id=eq.${nodeId}&select=spouse:relationship_undeployed_secondary_member_id_fkey(nodeId,name)`
      }
      console.log("URL->"+url)
